...
class printer  ()= object(self:'self)
  ...
  method longident_loc f x =
    fmt_comment f x.Asttypes.loc;
    (pp f "%a" self#longident x.txt)
  ...
  method signature_item f x :unit= begin
    ...
    | Psig_class l ->
        let class_description kwd f ({pci_params=ls;pci_name={txt;_};pci_loc;_} as x) =
          fmt_comment f pci_loc;
          pp f "@[<2>%s %a%a%s@;:@;%a@]%a" kwd
            self#virtual_flag x.pci_virt
            self#class_params_def ls txt
            self#class_type x.pci_expr
