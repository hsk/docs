...
and show_jvmprim x = Format.asprintf "%a" pp_jvmprim x

let rec pp_jprim fmt =
  function
  | `Int -> Format.pp_print_string fmt "`Int"
  ...
  | `Float -> Format.pp_print_string fmt "`Float"
  | `Double -> Format.pp_print_string fmt "`Double"
and show_jprim x = Format.asprintf "%a" pp_jprim x
...
let rec pp_jopcode fmt =
  ...
  | OpAdd a0 -> Format.fprintf fmt "(@[<hov2>JCode.OpAdd@ %a@])" pp_jvmprim a0
  | OpSub a0 -> Format.fprintf fmt "(@[<hov2>JCode.OpSub@ %a@])" pp_jvmprim a0
  | OpMul a0 -> Format.fprintf fmt "(@[<hov2>JCode.OpMul@ %a@])" pp_jvmprim a0
  | OpDiv a0 -> Format.fprintf fmt "(@[<hov2>JCode.OpDiv@ %a@])" pp_jvmprim a0
  | OpRem a0 -> Format.fprintf fmt "(@[<hov2>JCode.OpRem@ %a@])" pp_jvmprim a0
  | OpNeg a0 -> Format.fprintf fmt "(@[<hov2>JCode.OpNeg@ %a@])" pp_jvmprim a0
  ...
  | OpLookupSwitch (a0,a1) ->
      ...
      Format.fprintf fmt "@])"
  | OpReturn a0 ->
      Format.fprintf fmt "(@[<hov2>JCode.OpReturn@ %a@])" pp_jvmprim a0
  ...
  | OpInvokeSpecial a0 -> Format.fprintf fmt "(@[<hov2>JCode.OpInvokeSpecial@ %d@])" a0
  ...
  | OpNewArray a0 -> Format.fprintf fmt "(@[<hov2>JCode.OpNewArray@ %a@])" pp_jprim a0
