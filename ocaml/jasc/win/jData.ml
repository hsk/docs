...
let is_override_attrib = function
  | AttrVisibleAnnotations ann ->
    ...
    List.exists (function
      | { ann_type = TObject((["java";"lang"], "Override"), []) } -> true
      | _ -> false
    ) ann
  | _ -> false
...
let path_s (pack,name) =
  Printf.sprintf "%s.%s" (String.concat "." pack) name
...
and s_args = function
  ...
  | args ->
    let args = List.map (function
      ...
      | TType (WSuper,   s) -> "-" ^ (s_sig s)
    ) args in
    Printf.sprintf "<%s>" (String.concat ", " args)

let s_field f =
  Printf.sprintf "%s%s %s"
    (if is_override f then "override " else "")
    (s_sig f.jf_signature)
    f.jf_name
