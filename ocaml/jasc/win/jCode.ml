...
open ExtList
open IO.BigEndian
type other_num = [ `Long | `Float | `Double][@@deriving show]
...
type jvm_basic_type = [ `Int2Bool | other_num][@@deriving show]
...
type jopcodes = jopcode array[@@deriving show]
...
type exc_tbl =
  ...
  e_end: int;
  e_handler: int;
  e_catch_type: jpath option;}[@@deriving show]
...
type jcode =
  ...
  attributes: jattributes;}[@@deriving show]
let rec pp_jcode fmt x =
  ...
  (((((Format.pp_print_string fmt "JCode.max_stack = ";
   ...
   (pp_jattributes fmt) x.attributes);
  Format.fprintf fmt "@] }"
...
let get_code field =
  let rec loop =
    ...
    | _::xs -> loop xs in
  loop field.jf_attributes
File "jCode.ml", line 2, characters 5-14:
