...
ガーベジコレクションはとても便利で実行時に参照カウンタを更新する必要がありませんが、リアルタイム用を考えると難しい事が多いようです。
...
このとき、レベルが5のレベルより小さいポインタは親の世界なのでマークしません。下のコードで`*`がマークです。
...
マークが終わったら、世界を終わらせる特別なスイープをします。現状のレベルを1つ下げるのです。
...
ErlangのようなマルチVM<a name="r9"></a>[[9]](#9)は複数のVMを立ち上げアクター毎にメモリ管理を行うことが出来ます。
