...
Golangのgoルーチン[[8]](#8)や、Rustのspawn[[7]](#7)はメモリ空間を分けますが並列化の機能です。並列化にはスレッド起動かアクターのようなやり取りが必要で高コストです。
OCamlの多相的型推論はレベルを用いて解決しています[[6]](#6)が、その考えをオートリリースプールに応用出来るかもしれません。
