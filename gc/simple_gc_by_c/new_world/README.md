...
4. <a name="C5"></a>[実装](#c5)
5. <a name="C4"></a>[類似した実装](#c4)
6. <a name="C6"></a>[考察](#c6)
7. <a name="C7"></a>[今後の研究](#c7)
...
オートリリースプールの仕組みをGCに応用し、マイナーGCを小さい空間で行う高速なアルゴリズムが作ってみたいと思う訳です。
...
複数パスに別れるような計算では、段階ごとに作成したデータを次のパスへ渡すポイントでコピーするようにすれば、奇麗なGCが行えるのではないでしょうか。
...
新しい計算空間ではそのネストレベルを1上げ、終わったらレベルを1下げます。
計算が終った時には、計算結果をルート集合としてガーベジコレクションを行います。
ただし、マークやスイープを行うのは新しいヒープ空間内のみとします。
スイープ時には元の空間にコピーします。元のデータ空間には計算結果のみが増えます。
...
gc.cに実装があります。ここでは、mark\_and\_sweepとの差分を見て行きましょう。
