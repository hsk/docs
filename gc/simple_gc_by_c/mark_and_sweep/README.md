...
SML#のランタイムは様々なGCの実装を組み合わせて用いており参考になりました。
ビットマップを使う事でレコード内のデータがポインタかポインタでないかを示すテクニックも参考にしました。

これらのプログラムを元に、速度や互換性を犠牲にする事でより単純化することでより理解をしやすくしようとした物がここにあるサンプルプログラムです。
簡単なプログラムで動作を理解してしまえば、より複雑なアルゴリズムへ進む事が容易になるでしょう。
また、ここで用いたテクニックを使えばC言語へのトランスレータの出力結果が読みやすくなるかもしれません。
