...
しかし、理解すればC言語だけで完全なGCを使えることでしょう。トランスレータの出力を悩まず実装出来るはずです。
...
オブジェクトはユニオンで様々なデータのアクセスを楽にします。例えば、`a->intv`と書けば`int`型で取り出せ、`a->ints`と書く事でint型の配列として扱う事が出来ます。
...
`gc_free`はメモリを解放します。`frame_list`が`NULL`ならGCすれば全部消えるという訳です。
...
ここからは関数を使うテストです。まずは、低レベルに自力でフレームを作り、フレームリストに登録し、メモリアロケーションを行って、`gc`を呼び出し、最後にフレームリストを戻して終わります。目視でのテストは`assert`を入れて`heap`の数をチェックしています。
...
- データ構造
	...
	- enum ObjectType 
		...
		オブジェクトの種類を表します。
			...
			OBJ\_BOXED\_ARRAY   BOX化された配列
			OBJ\_UNBOXED\_ARRAY BOX化されていない配列
			OBJ\_PAIR           ペア
			OBJ\_RECORD         レコード
	...
	- struct ObjectHeader
		...
		`heap_list`に追加されています。GC時にマークされなかった物は削除されます。
		typeと、markは下位ビットに押し込めればよりメモり効率が良くなるでしょう。

	- union Object

		Objectは様々なデータの`union`です。
...
- グローバル変数
	...
	- Frame* frame_list
		...
		フレーム情報を格納します。`frame_prev`が前のフレームのアドレスをさします。
	...
	- int heap_max
		...
		ヒープ内のデータの最大値です。初期値は8で、`heap_max`に到達すると、gcが実行され、倍のサイズに拡張されます。
...
- 関数
	...
	- GC
		...
		- void* gc\_alloc\_int(int n)
			...
			`int`の値を持ったオブジェクトを作成します。UNBOXEDな配列を使っています。
