## mark bitをSetを使って実装する setgc

上の実装では、markbitを各データに持たせていました。
このマークビットを別のセットに保存する事にしてみましょう。
...
Setを使ってcompareはObj.magicの値を使いました。プログラム実行時はオブジェクトはオブジェクトのリストに溜め込まれます。
リストのサイズが一定量を超えると、gcが動き、markフェーズではsetにオブジェクトが溜め込まれます。
sweepフェーズではセットにそのオブジェクトが含まれていれば、消さずにのこし、セットになければ消すだけです。
