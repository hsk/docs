...
object XXMLParser extends RegexParsers {
  ...
  def html = otag("html", head ~ body ^^ {case a~b=>a:::b})
  def head = otag("head",r(
    ones(List("base","bgsound","isindex","nextid","command","link","meta")) |
    tag("title", texts) |
    tag("noscript", r(inline)) |
    script
  ))
  def body = otag("body", r(flow))

  def flow:Parser[List[XXML]] =
    one("hr") |
    ...
    heading | p | inline
  def heading:Parser[List[XXML]] =
    tags(List("h1","h2","h3","h4","h5","h6"), r(not(heading) ~> flow))
  def p:Parser[List[XXML]] = tago("p", r(not(p) ~> inline))
  def tr = tago("tr",r(tago("td", r(flow)) | tago("th", r(flow))))

  def inline:Parser[List[XXML]] = text |
  ...
  def option = tago("option", texts)
  def script = tags(List("script", "template", "style"), cdata)
}
