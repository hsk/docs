...
パーサコンビネータによる実装は必ずしも効率的ではないかもしれません。
より効率的なパーサの検討が必要でしょう。
エスケープシーケンスについての処理は行いませんでしたのでエスケープシーケンスの処理も追加すると良いはずです。
