...
object XXMLParser extends RegexParsers {
  ...
  def tag(name1:String, p: Parser[List[XXML]]) =
    if (name1.length == 1)
      com('<' ~> name1.charAt(0) <~ "/>" ^^ {a=>Block(name1, List())} |
          '<' ~> name1.charAt(0) ~> '>' ~> p <~ rep("</" ~ (not(name1 | '>')~name)  ~> '>') <~ opt("</" <~ opt(name1) <~ '>') ^^
          {a=>Block(name1, a)})
    else
      com(("<" + name1) <~ "/>" ^^ {a=>Block(name1, List())} |
          ("<" + name1) ~> '>' ~> p <~ rep("</" ~ (not(name1 | '>')~name)  ~> '>') <~ opt("</" <~ opt(name1) <~ '>') ^^
      {a=>Block(name1, a)})
  // 省略出来ないタグ一括登録
  ...
  def tago(name:String, p: Parser[List[XXML]]) =
    if (name.length == 1)
      com('<' ~> name.charAt(0) <~ "/>" ^^ {a=>Block(name, List())} |
          '<' ~> name.charAt(0) ~> '>' ~> p <~ opt("</" <~ opt(name) <~ '>') ^^ {Block(name, _)})
    else
      com(("<" + name) <~ "/>" ^^ {a=>Block(name, List())} |
  ...
  lazy val html = otag("html", head ~ body ^^ {case a~b=>a:::b})
  ...
  lazy val flow:Parser[List[XXML]] =
    one("hr") |
